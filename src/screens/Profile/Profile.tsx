...
const ProfileField = ({ label, placeholder, value, onChange, type = "text", prefix }: { label: string; placeholder: string; value: string; onChange: (val: string) => void; type?: string; prefix?: string }) => (
    <div className="mb-6">
        <label className="block text-3 font-bold text-taxable-dark mb-3">{label}</label>
        <div className="relative">
            {prefix && (
                <div className="absolute left-4 top-1/2 -translate-y-1/2 text-neutral-400 font-medium">{prefix}</div>
            )}
            <input
                type={type}
                value={value}
                ...
                className={`w-full h-14 bg-white border border-neutral-100 rounded-xl px-4 text-taxable-dark focus:outline-none focus:ring-1 focus:ring-taxable-blue/20 placeholder:text-neutral-300 font-medium ${prefix ? 'pl-14' : ''}`}
            />
        </div>
    </div>
);
...
const Toggle = ({ enabled, onChange }: { enabled: boolean; onChange: () => void }) => (
    <button
        onClick={onChange}
        className={`w-[44px] h-[24px] rounded-full relative transition-colors duration-200 ${enabled ? 'bg-taxable-blue' : 'bg-slate-200'}`}
    >
            ...
            <main className="max-w-[1240px] mx-auto pt-8 md:pt-12 pb-20 px-4 md:px-8">
                ...
                <div className="flex items-start justify-between mb-12">
                    <div>
                        ...
                        <p className="text-3 md:text-lg text-taxable-gray font-medium max-w-[500px] leading-relaxed">
                    ...
                    <div className="relative">
                        <button
                            ...
                            className="h-[48px] md:h-[52px] px-5 md:px-7 bg-white border border-neutral-100 rounded-2xl text-[14px] md:text-3 font-bold text-taxable-dark shadow-xs hover:shadow-md transition-all flex items-center gap-2"
                        >
                            Contact support
                        </button>
                ...
                <div className="flex flex-col lg:flex-row lg:justify-center gap-10 lg:gap-16">
                    {/* Sidebar */}
                    <div className="w-full lg:w-[303px] h-auto lg:h-[218px] bg-white rounded-[20px] border border-neutral-100 p-3 shadow-sm flex flex-col shrink-0 lg:sticky lg:top-24">
                        ...
                        <div className="flex flex-col gap-1 items-center">
                            {categories.map((category) => (
                                <button
                                    ...
                                    className={`w-full lg:w-[279px] h-[44px] lg:h-[37px] flex items-center justify-between px-3 transition-all rounded-lg ${activeSection === category ? 'bg-slate-100 text-taxable-dark font-bold' : 'hover:bg-neutral-50 text-taxable-gray'
                                        }`}
                                >
                                    <span className={`text-sm ${activeSection === category ? 'font-semibold' : 'font-medium'}`}>{category}</span>
                                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" className={activeSection === category ? 'text-taxable-dark' : 'text-neutral-300'}>
                    ...
                    <div className="flex-1 w-full max-w-full lg:max-w-[480px] lg:mx-auto">
                        {activeSection === 'Personal Information' && (
                            <div className="animate-in fade-in duration-500">
                                ...
                                <div className="flex items-center gap-6 mb-8 pb-8 border-b border-neutral-50">
                                    <input
                                        type="file"
                                        ref={fileInputRef}
                                        ...
                                        accept="image/*"
                                    />
                                    <div className="w-20 h-20 rounded-3xl bg-stone-100 flex items-center justify-center shrink-0 overflow-hidden border border-neutral-50 relative">
                                        <Image
                                    ...
                                    <div>
                                        <h3 className="text-4 font-bold text-taxable-dark mb-1">Upload Image</h3>
                                        <p className="text-2 text-taxable-gray font-medium mb-3">Min 400x400px, PNG or JPEG</p>
                                        <button
                                            onClick={() => fileInputRef.current?.click()}
                                            className="h-10 px-6 border border-neutral-100 rounded-xl text-[14px] font-bold text-taxable-dark hover:bg-neutral-50 transition-colors"
                                        >
                                            Upload
                                        </button>
                                    </div>
                                ...
                                <div className="flex items-center gap-3 mt-8">
                                    <button
                                        ...
                                        className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${formData.whatsappReminders ? 'bg-taxable-blue border-taxable-blue' : 'border-neutral-200 bg-white'}`}
                                    >
                                ...
                                <div className="flex items-center gap-3 mt-10">
                                    <button
                                        ...
                                        className="h-[52px] px-8 border border-neutral-100 text-taxable-dark font-bold rounded-xl hover:bg-neutral-50 transition-colors"
                                    >
                                        Reset
                                    </button>
                                    <button
                                        ...
                                        disabled={apiLoading}
                                        className="h-[52px] px-10 bg-taxable-blue text-white font-bold rounded-xl hover:opacity-95 transition-opacity shadow-sm disabled:opacity-50"
                                    >
                            ...
                            <div className="animate-in fade-in duration-500 space-y-10">
                                <div className="space-y-4">
                                    ...
                                    <div className="relative">
                                        ...
                                        />
                                        <button className="absolute right-4 top-[50px] text-neutral-400">
                                    ...
                                    <div className="flex items-center gap-2 -mt-2">
                                        ...
                                        <p className="text-1 text-taxable-gray font-medium">At least 8 characters. Make it strong!</p>
                                    </div>
                                </div>
                                ...
                                <div className="flex items-center gap-3 pt-2">
                                    ...
                                    <button
                                        ...
                                        className="h-[52px] px-10 bg-taxable-blue text-white font-bold rounded-xl hover:opacity-95 transition-opacity disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                                    >
                                        {apiLoading ? (
                                            <>
                                ...
                                <div className="pt-8 border-t border-neutral-50">
                                    <h2 className="text-5 font-bold text-taxable-dark mb-4">Two-Factor Authentication (2FA)</h2>

                                    <div className="bg-slate-50 rounded-3xl p-6 flex gap-4 mb-8">
                                        ...
                                        <div>
                                            <h4 className="text-3 font-bold text-taxable-dark mb-1">Why you need this</h4>
                                    ...
                                    <div className="space-y-6 mb-10">
                                        <label className="flex items-start gap-3 cursor-pointer group">
                                            <div onClick={() => setSecurityData({ ...securityData, tfaType: 'authenticator' })} className={`w-5 h-5 rounded-full border-2 flex items-center justify-center mt-1 transition-colors ${securityData.tfaType === 'authenticator' ? 'border-taxable-blue' : 'border-neutral-200'}`}>
                                                {securityData.tfaType === 'authenticator' && <div className="w-[10px] h-[10px] bg-taxable-blue rounded-full" />}
                                            </div>
                                            <div>
                                                <h4 className="text-3 font-bold text-taxable-dark leading-none mb-2">Authenticator App (Recommended)</h4>
                                                <p className="text-2 text-taxable-gray font-medium">Use an app like Google Authenticator, Authy, or Microsoft Authenticator</p>
                                            </div>
                                        </label>

                                        <label className="flex items-start gap-3 cursor-pointer group">
                                            <div onClick={() => setSecurityData({ ...securityData, tfaType: 'sms' })} className={`w-5 h-5 rounded-full border-2 flex items-center justify-center mt-1 transition-colors ${securityData.tfaType === 'sms' ? 'border-taxable-blue' : 'border-neutral-200'}`}>
                                                {securityData.tfaType === 'sms' && <div className="w-[10px] h-[10px] bg-taxable-blue rounded-full" />}
                                            </div>
                                            <div>
                                                <h4 className="text-3 font-bold text-taxable-dark leading-none mb-2">SMS Text Message</h4>
                                                <p className="text-2 text-taxable-gray font-medium">Receive codes via SMS (requires verified phone)</p>
                                            </div>
                                        </label>
                                    </div>
                                    ...
                                    {securityData.isEnabling && !user?.twoFactorEnabled ? (
                                        <div className="space-y-6 animate-in fade-in slide-in-from-top-4 duration-300">
                                            {tfaSetupData && (
                                                <div className="bg-white border border-neutral-100 rounded-4xl p-8 flex flex-col items-center shadow-sm">
                                                    <div className="w-12 h-12 bg-blue-50 rounded-2xl flex items-center justify-center mb-6">
                                                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" className="text-taxable-blue" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                                                    ...
                                                    <div className="bg-white p-4 rounded-3xl border-4 border-neutral-50 mb-8">
                                                        <QRCode
                                                    ...
                                                    <div className="w-full space-y-4">
                                                        <div className="p-4 bg-blue-50/50 rounded-2xl border border-blue-100/50">
                                                            ...
                                                            <div className="flex items-center justify-between">
                                                                <code className="text-2 font-mono font-bold text-taxable-dark">{tfaSetupData.manualEntryKey}</code>
                                                                <button
                                                                    onClick={() => {
                                                                    ...
                                                                    className="text-taxable-blue text-1 font-bold hover:underline"
                                                                >
                                                                    Copy
                                                                </button>
                                                            </div>
                                            ...
                                            <div className="pt-4">
                                                <div className="mb-6">
                                                    ...
                                                    <div className="flex justify-center">
                                                        <input
                                                            ...
                                                            maxLength={6}
                                                            placeholder="000000"
                                                            className="w-48 h-14 bg-white border border-neutral-100 rounded-2xl text-center text-2xl font-bold tracking-[0.5em] text-taxable-dark focus:outline-none focus:ring-2 focus:ring-taxable-blue/20 placeholder:text-neutral-200"
                                                ...
                                                <div className="flex items-center gap-3">
                                                    <button
                                                        ...
                                                        className="flex-1 h-14 bg-white border border-neutral-100 text-taxable-dark font-bold rounded-2xl hover:bg-neutral-50 transition-all"
                                                    >
                                                        Cancel
                                                    </button>
                                                    <button
                                                        ...
                                                        className="flex-[1.5] h-14 bg-taxable-blue text-white font-bold rounded-2xl hover:opacity-95 transition-all shadow-lg shadow-blue-900/10 disabled:opacity-50 disabled:shadow-none flex items-center justify-center gap-2"
                                                    >
                                                        {apiLoading ? (
                                        ...
                                        <div className="flex flex-col gap-4">
                                            {user?.twoFactorEnabled ? (
                                                <div className="bg-white border border-neutral-100 rounded-3xl p-6 flex flex-col items-center">
                                                    <div className="w-12 h-12 bg-green-50 rounded-2xl flex items-center justify-center mb-4">
                                                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" className="text-emerald-500" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                                                ...
                                                <button
                                                    ...
                                                    className="h-[52px] px-8 bg-white border border-neutral-100 text-taxable-dark font-bold rounded-xl hover:bg-neutral-50 transition-colors shadow-sm"
                                                >
                                                    Enable 2FA
                                                </button>
                                            )}
                            ...
                            <div className="animate-in fade-in duration-500 max-w-[540px]">
                                ...
                                <div className="space-y-6">
                                    {[
                                        ...
                                        <div key={item.key} className="flex items-center justify-between pb-4 border-b border-neutral-50 last:border-0">
                                            <span className="text-3 font-medium text-taxable-dark">{item.label}</span>
                                            <Toggle
