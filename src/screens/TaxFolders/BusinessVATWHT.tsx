                        ...
                        return (
                            <div key={item.key}>
                                <button
                                    ...
                                    className={`w-full flex items-center justify-between px-3 py-2.5 rounded-xl transition-all mb-0.5 ${isActive ? 'hover:bg-neutral-50' : 'hover:bg-neutral-50'}`}
                                >
                                    <div className="flex items-center gap-3 text-left">
                                        <span className="text-lg leading-none">📁</span>
                                        <span className={`text-2 font-semibold ${isActive ? 'text-neutral-800' : 'text-neutral-700'}`}>
                                            {item.label}
                                        </span>
                                    </div>
                                    {isExpanded ? (
        ...
        {MONTHS_SHORT.slice(0, 3).map((m, i) => {
            ...
            return (
                <button key={m} onClick={() => setActiveMonth(i)}
                    className={`flex items-center gap-2 px-2 py-2.5 rounded-lg text-2 font-semibold whitespace-nowrap transition-all ${isActive ? 'bg-slate-100 text-neutral-800' : 'text-neutral-400 hover:text-neutral-500'}`}>
                    {filed
                        ? <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" className="text-green-600" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12" /></svg>
    ...
    return (
        <div>
            ...
            <div className="bg-white border border-neutral-200 rounded-2xl p-5 mb-4">
                <h3 className="text-2 font-bold text-neutral-800 mb-4">Payee Details (who you paid)</h3>
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        <label className="flex items-center text-1 font-bold text-neutral-500 mb-1.5">
                        ...
                        <input type="text" placeholder="N0" value={form.payee}
                            ...
                            className="w-full h-10 border border-neutral-200 bg-neutral-50 rounded-xl px-3 text-2 font-medium text-neutral-800 placeholder:text-neutral-300 focus:outline-none focus:border-taxable-blue/40" />
                    </div>
                    <div>
                        <label className="flex items-center text-1 font-bold text-neutral-500 mb-1.5">
            ...
            <div className="bg-white border border-neutral-200 rounded-2xl p-5 mb-5">
                <h3 className="text-2 font-bold text-neutral-800 mb-4">Payment Details</h3>
                <div className="space-y-3">
                    ...
                    <div className="flex items-center justify-between gap-4">
                        ...
                        <div className="w-[220px] flex-shrink-0 h-10 border border-neutral-200 bg-neutral-50 rounded-xl px-3 flex items-center text-2 font-medium text-neutral-500">
            ...
            <div className="flex gap-3">
                <button onClick={onCancel} className="flex-1 h-11 border border-neutral-300 rounded-xl text-[14px] font-bold text-neutral-800 hover:bg-neutral-50 transition-colors">Cancel</button>
                <button onClick={() => onSave({ ...form, whtDeducted: String(Math.round(autoWHT)), netPaid: String(Math.round(autoNet)), whtRate: String(autoRate) })}
                    className="flex-[2] h-11 bg-taxable-blue text-white rounded-xl text-[14px] font-bold hover:opacity-90 transition-opacity">
                    Save WHT Deduction
                </button>
            </div>
        </div>
    ...
    return (
        <div className="bg-white border border-neutral-200 rounded-2xl p-5">
            <div className="grid grid-cols-3 gap-4 mb-3">
                <div><p className="text-[11px] font-semibold text-neutral-400 mb-0.5">Payee</p><p className="text-2 font-semibold text-neutral-800">{d.payee || 'N/A'}</p></div>
                <div><p className="text-[11px] font-semibold text-neutral-400 mb-0.5">TIN</p><p className="text-2 font-semibold text-neutral-800 font-mono">{d.tin || 'N/A'}</p></div>
                <div><p className="text-[11px] font-semibold text-neutral-400 mb-0.5">Type</p><p className="text-2 font-semibold text-neutral-800">{d.whtType !== 'Select' ? d.whtType : 'N/A'}</p></div>
            </div>
            <div className="grid grid-cols-4 gap-4 mb-3">
                <div><p className="text-[11px] font-semibold text-neutral-400 mb-0.5">Gross</p><p className="text-2 font-bold text-neutral-800">{fmt(gross)}</p></div>
                <div><p className="text-[11px] font-semibold text-neutral-400 mb-0.5">WHT</p><p className="text-2 font-bold text-neutral-800">{fmt(wht)}</p></div>
                <div><p className="text-[11px] font-semibold text-neutral-400 mb-0.5">Net</p><p className="text-2 font-bold text-neutral-800">{fmt(net)}</p></div>
                <div><p className="text-[11px] font-semibold text-neutral-400 mb-0.5">Date</p><p className="text-2 font-semibold text-neutral-800">{d.date || '—'}</p></div>
            </div>
            <div className="flex gap-2">
                <button onClick={onRemove} className="flex-1 h-9 border border-neutral-200 rounded-xl text-1 font-bold text-neutral-800 hover:bg-neutral-50 transition-colors">Remove</button>
                <button onClick={onEdit} className="flex-1 h-9 border border-neutral-200 rounded-xl text-1 font-bold text-neutral-800 hover:bg-neutral-50 transition-colors">Edit</button>
            </div>
        </div>
    );
};
        ...
        <div className="flex gap-6">
            ...
            <div className="flex-1 min-w-0">
                {showForm || editId !== null ? (
                    ...
                    <div>
                        <h2 className="text-[16px] font-bold text-neutral-800 mb-5">Payee Details (who you paid)</h2>
                        ...
                        <div className="mb-2">
                            ...
                            <p className="text-[32px] font-bold text-neutral-800">₦{Math.round(total).toLocaleString()}</p>
                        </div>
                        ...
                        <div className="flex gap-3">
                            ...
                            <button onClick={() => setShowFilingModal(true)}
                                className="flex-1 h-12 bg-taxable-blue text-white font-bold rounded-xl hover:opacity-90 transition-opacity text-[14px]">
            ...
            <div className="flex-1 min-w-0">
                {(showForm || editId !== null) ? (
                    ...
                    <div className="flex flex-col items-start gap-3">
                        <h2 className="text-[16px] font-bold text-neutral-800">WHT Credit Notes</h2>
                        <p className="text-2 text-neutral-500 font-medium">WHT deducted from payments received — used to offset your CIT liability.</p>
                        <button onClick={() => setShowForm(true)}
                            className="mt-2 flex items-center gap-1.5 text-2 font-bold text-taxable-blue hover:opacity-80 transition-opacity">
                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><line x1="12" y1="5" x2="12" y2="19" /><line x1="5" y1="12" x2="19" y2="12" /></svg>
                            Add WHT Credit Note
                        </button>
                    </div>
                    ...
                    <div>
                        ...
                        <div className="mb-6 bg-blue-50 border border-blue-100 rounded-2xl p-5 inline-flex gap-8">
                            <div>
                                <p className="text-[11px] font-semibold text-neutral-500 mb-1">Total WHT Credit</p>
                                <p className="text-7 font-bold text-taxable-blue">₦{Math.round(total).toLocaleString()}</p>
                            </div>
                            <button className="self-center h-10 px-5 bg-taxable-blue text-white font-bold rounded-xl hover:opacity-90 transition-opacity text-2">
                                Apply to CIT
                            </button>
                        </div>
                    </div>
            ...
            <div className="flex-1 min-w-0">
                ...
                {subSection === 'file-vat' && vatStep === 'method' && (
                    <div className="max-w-[480px] mx-auto">
                        <h2 className="text-base font-bold text-neutral-800 mb-1">File Monthly VAT Return</h2>
                        <p className="text-2 text-neutral-500 font-medium mb-6">How do you want to enter your VAT data?</p>
                        ...
                        <button onClick={() => setVatStep('form')} className="h-11 px-8 bg-taxable-blue text-white font-bold rounded-xl hover:opacity-90 transition-opacity text-[14px]">
                            Continue
                        </button>
                    </div>
                )}
                        ...
                        <div className="flex-1 min-w-0 space-y-6 pb-6">
                            ...
                            <div className="bg-white border border-neutral-200 rounded-2xl overflow-hidden">
                                <div className="px-5 py-3.5 border-b border-neutral-100">
                                    <h3 className="text-[14px] font-bold text-neutral-800">Output VAT <span className="text-neutral-500 font-medium">(VAT you charged customers)</span></h3>
                                </div>
                                <div className="p-5 space-y-3">
                                    {/* Total sales */}
                                    <div className="flex items-center justify-between gap-4">
                                        <label className="text-2 font-semibold text-neutral-700 flex items-center whitespace-nowrap flex-shrink-0">
                                        ...
                                        <input
                                            ...
                                            placeholder="N0"
                                            className="w-[160px] flex-shrink-0 h-10 border border-neutral-200 bg-neutral-50 rounded-xl px-3 text-2 font-medium text-neutral-800 placeholder:text-neutral-300 focus:outline-none focus:border-taxable-blue/40 text-right"
                                        />
                                    </div>
                                    {/* VAT charged */}
                                    <div className="flex items-center justify-between gap-4">
                                        ...
                                        <div className="w-[160px] flex-shrink-0 h-10 border border-neutral-200 bg-neutral-50 rounded-xl px-3 flex items-center justify-end text-2 font-medium text-neutral-800">
                                    ...
                                    <div className="flex items-center justify-between gap-4">
                                        ...
                                        <div className="relative w-[160px] flex-shrink-0">
                                            <select
                                                ...
                                                className="w-full h-10 border border-neutral-200 bg-neutral-50 rounded-xl px-3 text-2 font-medium text-neutral-800 focus:outline-none focus:border-taxable-blue/40 appearance-none"
                                            >
                                    ...
                                    {/* Upload */}
                                    <div className="flex items-center justify-between gap-4 mt-2 p-3 border border-dashed border-neutral-200 rounded-xl">
                                        ...
                                        <button className="h-8 px-4 border border-neutral-300 rounded-lg text-1 font-bold text-neutral-800 hover:bg-neutral-50 transition-colors">
                                            Upload
                                        </button>
                                    </div>
                                    ...
                                    {/* Total output */}
                                    <div className="pt-2">
                                        <p className="text-[11px] font-semibold text-neutral-500 mb-0.5">Total output VAT</p>
                                        <p className="text-base font-bold text-neutral-800">{fmt(outputVAT)}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="bg-white border border-neutral-200 rounded-2xl overflow-hidden">
                                <div className="px-5 py-3.5 border-b border-neutral-100">
                                    <h3 className="text-[14px] font-bold text-neutral-800">Input VAT <span className="text-neutral-500 font-medium">(VAT you paid to suppliers)</span></h3>
                                </div>
                                <div className="p-5 space-y-3">
                                    {/* Total purchases */}
                                    <div className="flex items-center justify-between gap-4">
                                    ...
                                    {/* Total input */}
                                    <div className="pt-2">
                                        <p className="text-[11px] font-semibold text-neutral-500 mb-0.5">Total input VAT</p>
                                        <p className="text-base font-bold text-neutral-800">{fmt(inputVAT)}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            {/* Net VAT Payable */}
                            <div>
                                <h3 className="text-sm font-bold text-neutral-800 mb-4">Net VAT Payable</h3>
                                <div className="space-y-2.5 mb-4">
                                    ...
                                    <div className="flex items-center justify-between text-2">
                                        ...
                                        <span className="font-semibold text-neutral-800">-{fmt(inputVAT)}</span>
                                    </div>
                                    <div className="flex items-center justify-between text-[14px] pt-2 border-t border-neutral-100">
                                        <span className="font-bold text-neutral-800">Net VAT {isCredit ? 'credit' : 'due'}</span>
                                        <span className={`font-bold text-4 ${isCredit ? 'text-green-600' : 'text-neutral-800'}`}>
                                    ...
                                    {isCredit ? (
                                        <>
                                            <button onClick={() => { setFiledMonths(prev => new Set([...prev, activeMonth])); if (activeMonth < 11) setActiveMonth(m => m + 1); }}
                                                className="flex-1 h-12 border border-neutral-300 text-neutral-800 font-bold rounded-xl hover:bg-neutral-50 transition-colors text-[14px]">
