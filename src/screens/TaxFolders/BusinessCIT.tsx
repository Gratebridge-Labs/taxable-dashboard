                        ...
                        return (
                            <div key={item.key}>
                                <button
                                    ...
                                    className="w-full flex items-center justify-between px-3 py-2.5 rounded-xl transition-all mb-0.5 hover:bg-gray-50"
                                >
                                    <div className="flex items-center gap-3 text-left">
                                        <span className="text-lg leading-none">📁</span>
                                        <span className="text-[13px] font-semibold text-[#374151]">{item.label}</span>
                                    </div>
                                    {isExpanded ? (
                                    ...
                                    <div className="ml-9 mb-1">
                                        {item.children.map(child => (
                                            <button
                                                ...
                                                className={`w-full text-left px-3 py-2 rounded-lg text-[12.5px] font-semibold transition-colors mb-0.5 ${activeSubSection === child.key ? 'text-[#0C0C0E] bg-[#F1F5F9]' : 'text-[#6B7280] hover:text-[#374151] hover:bg-gray-50'}`}
                                            >
                                                {child.label}
                                            </button>
                                        ))}
                ...
                <div className="flex-1 px-6 py-5">
                    {OPTIONS.map(opt => (
                        <button key={opt.id} onClick={() => setMethod(opt.id)} className="w-full flex items-start gap-3 py-4 text-left">
                            <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 mt-0.5 ${method === opt.id ? 'border-[#003787]' : 'border-gray-300'}`}>
...
const Field = ({ label, tip, value, onChange, readOnly, placeholder }: {
    ...
    <div>
        <label className="flex items-center text-[12px] font-semibold text-[#6B7280] mb-1.5">
        ...
        <input
            ...
            className={`w-full h-10 border border-gray-200 rounded-xl px-3 text-[13px] font-medium text-[#0C0C0E] placeholder:text-gray-300 focus:outline-none focus:border-[#003787]/40 transition-all ${readOnly ? 'bg-[#F9FAFB] text-[#9CA3AF] cursor-default' : 'bg-[#F9FAFB]'}`}
        />
    </div>
);
...
const LedgerRow = ({ label, value, bold, indent, prefix }: {
    ...
    <div className={`flex items-center justify-between py-2.5 ${bold ? 'border-t border-gray-100 mt-1' : ''}`}>
        <span className={`text-[13px] ${bold ? 'font-bold text-[#0C0C0E]' : 'font-medium text-[#6B7280]'} ${indent ? 'pl-4' : ''}`}>{label}</span>
        <span className={`text-[13px] ${bold ? 'font-bold text-[#0C0C0E]' : 'font-semibold text-[#0C0C0E]'}`}>
            {prefix}{value}
        </span>
    </div>
);
...
const SectionHeader = ({ label }: { label: string }) => (
    <p className="text-[13px] font-bold text-[#0C0C0E] mt-5 mb-1">{label}</p>
);
            ...
            <div className="flex-1 min-w-0">
                    ...
                    return (
                        <div className="max-w-[620px]">
                            ...
                            {showDeferModal && deferModalQuarter !== null && (
                                <div className="fixed inset-0 z-[200] flex items-center justify-center">
                                    ...
                                    <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-[380px] mx-4 p-7 text-center">
                                        ...
                                        <h3 className="text-[16px] font-bold text-[#0C0C0E] mb-3">Defer to annual filing</h3>
                                        ...
                                        <button
                                            ...
                                            className="w-full h-12 bg-[#003787] text-white rounded-xl font-bold text-[14px] hover:opacity-90 transition-opacity">
                                            Got it
                                        </button>
                                    </div>
                                </div>
                            ...
                            <div className="mb-5 space-y-2">
                                ...
                                <div className="flex items-center justify-between text-[13px]">
                                    <span className="text-[#6B7280] font-medium">Estimated CIT (30%)</span>
                                    <span className="font-semibold text-[#0C0C0E]">{qFmt(totalCIT)}</span>
                                </div>
                                <div className="flex items-center justify-between text-[13px]">
                                    <span className="text-[#6B7280] font-medium">Per quarter</span>
                                    <span className="font-semibold text-[#0C0C0E]">{qFmt(perQuarter)}</span>
                                </div>
                            </div>
                            ...
                            {editingEstimate ? (
                                <div className="flex items-center gap-3 mb-6">
                                    <input type="text" value={editEstimateVal}
                                        ...
                                        className="flex-1 h-10 border border-gray-200 bg-[#F9FAFB] rounded-xl px-3 text-[13px] font-medium focus:outline-none focus:border-[#003787]/40" />
                                    <button onClick={() => { if (editEstimateVal) setEstimatedProfit(editEstimateVal); setEditingEstimate(false); }}
                                        className="h-10 px-4 bg-[#003787] text-white rounded-xl text-[13px] font-bold hover:opacity-90">Save</button>
                                    <button onClick={() => setEditingEstimate(false)}
                                        className="h-10 px-4 border border-gray-200 rounded-xl text-[13px] font-bold hover:bg-gray-50">Cancel</button>
                                </div>
                            ) : (
                                                ...
                                                <tr key={q.label} className="hover:bg-[#FAFAFA] transition-colors">
                                                    ...
                                                    <td className="px-5 py-3.5">
                                                        {isPaid ? (
                                                            ...
                                                            <span className="flex items-center gap-1.5 text-[#16A34A] font-medium text-[12px]">
                                                                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12" /></svg>
                                                            ...
                                                            <div className="flex items-center gap-3">
                                                                <button
                                                                    ...
                                                                    className="flex items-center gap-1 text-[#003787] font-bold text-[12px] hover:opacity-80 transition-opacity">
                                                                    Pay & File Q{i + 1}
                                                                ...
                                                                <button
                                                                    ...
                                                                    className="text-[#6B7280] font-semibold text-[12px] hover:text-[#374151] transition-colors">
                                                                    Defer to annual filing
                                                                </button>
                                                            </div>
                                                        ) : (
                                                            <span className="text-[#9CA3AF] font-medium text-[12px]">Upcoming</span>
                                                        )}
                                                    </td>
                                                </tr>
                                            );
                            ...
                            {allPaid ? (
                                <div className="space-y-4">
                                    ...
                                    <button onClick={() => setSubSection('file-returns')} className="h-12 px-8 bg-[#003787] text-white font-bold rounded-xl hover:opacity-90 transition-opacity text-[14px]">
                                        File Annual CIT
                                    </button>
                                </div>
                            ) : allSettled ? (
                                <div className="space-y-4">
                                    <div className="flex items-start gap-2">
                                        ...
                                        <p className="text-[12px] font-semibold text-[#D97706] leading-relaxed">
                ...
                {subSection === 'file-returns' && step === 'method' && (
                    <div className="max-w-[520px] mx-auto">
                        ...
                        <button onClick={() => setStep('form')} className="h-11 px-8 bg-[#003787] text-white font-bold rounded-xl hover:opacity-90 transition-opacity text-[14px]">
                            Continue
                        </button>
                    </div>
                )}
                            ...
                            <div>
                                ...
                                <div className="flex items-center justify-between gap-4">
                                    ...
                                    <input type="text" placeholder="N0" value={nonDeductible}
                                        ...
                                        className="w-[180px] flex-shrink-0 h-10 border border-gray-200 bg-[#F9FAFB] rounded-xl px-3 text-[13px] font-medium text-[#0C0C0E] placeholder:text-gray-300 focus:outline-none focus:border-[#003787]/40" />
                                </div>
                            </div>

                            <div className="border-t border-gray-100 pt-4">
                                <p className="text-[13px] font-bold text-[#0C0C0E] mb-3">Subtract</p>
                        ...
                        {whtCredits.map((credit, idx) => (
                            <div key={idx} className="bg-white border border-gray-200 rounded-2xl p-5 mb-4">
                                <div className="space-y-3">
                                    ...
                                    {/* Upload */}
                                    <div className="flex items-center justify-between gap-4 p-3 border border-dashed border-gray-200 rounded-xl mt-1">
                                        <div className="flex items-center gap-2.5">
                                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#9CA3AF" strokeWidth="2">
                                                <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" /><polyline points="14 2 14 8 20 8" />
                                            </svg>
